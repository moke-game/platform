...
func (c *partyServiceClient) JoinParty(ctx context.Context, in *JoinPartyRequest, opts ...grpc.CallOption) (PartyService_JoinPartyClient, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &PartyService_ServiceDesc.Streams[0], PartyService_JoinParty_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &partyServiceJoinPartyClient{ClientStream: stream}
...
type partyServiceJoinPartyClient struct {
	grpc.ClientStream
}

func (x *partyServiceJoinPartyClient) Recv() (*JoinPartyResponse, error) {
	m := new(JoinPartyResponse)
...
func (UnimplementedPartyServiceServer) JoinParty(*JoinPartyRequest, PartyService_JoinPartyServer) error {
...
func _PartyService_JoinParty_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(PartyServiceServer).JoinParty(m, &partyServiceJoinPartyServer{ServerStream: stream})
}
...
type partyServiceJoinPartyServer struct {
	grpc.ServerStream
}

func (x *partyServiceJoinPartyServer) Send(m *JoinPartyResponse) error {
